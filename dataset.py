...
class BaseRetina(Dataset):
    def __init__(self, path):
        self.img_list = glob.glob(os.path.join(path, "*.png"))
        self.img_list = natsort.natsorted(self.img_list)
        print(self.img_list)
    ...
    def __getitem__(self, index):
        ...
        mask = Image.open(self.img_list[index].replace("train/img", "train/mask")).convert("L")
        color = np.load(self.img_list[index].replace("train/img", "train/color").replace(".png", ".npy"))
        fovea = np.load(self.img_list[index].replace("train/img", "train/fovea_256").replace(".png", ".npy"))
        od = np.load(self.img_list[index].replace("train/img", "train/od_256").replace(".png", ".npy"))
        style = np.load(self.img_list[index].replace("train/img", "train/style").replace(".png", ".npy"))
        disease = np.load(self.img_list[index].replace("train/img", "train/disease_ebd").replace(".png", ".npy"))
        
        # print(index)
        return img, mask, color, fovea, od, style, disease
    
class Retina(Dataset):
    def __init__(self,
                 path=os.path.expanduser('/data/sunghun/dataset/new_full_data/train/img'),
                 image_size=256,
    ...
    def __getitem__(self, index):
        img, mask, color, fovea, od, style, disease = self.data[index]
        ...
        # km = KMeans(n_clusters=8, n_init=10) # to do : 8을 여기서 선언하는 게 아니라 config로 뺄 것
        # km.fit(pixels)
        # colors = np.asarray(km.cluster_centers_, dtype='uint8')
        # colors = transforms.ToTensor()(colors) # shape : [1,8,3]
        # colors = colors.reshape(-1)

        color = (torch.from_numpy(color)/255.0).half()
        fovea = (torch.from_numpy(fovea)/256.0).half()
        od = (torch.from_numpy(od)/256.0).half()
        style = torch.from_numpy(style).half()
        disease = torch.from_numpy(disease).half()

        # tf_toTensor = transforms.ToTensor()
        # img = (self.transform(img)+self.transform(mask))/2.0
        img = self.transform(img)
        mask = self.transform(mask)
        # print("dataset", img.shape, mask.shape)
        return {'img': img, 'index': index, 'mask': mask, 'color': color, 'fovea': fovea, 'od': od, 'style': style, 'disease': disease}
